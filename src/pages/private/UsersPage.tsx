...
import { UserDialog, UserFilter, UserHeader, UserTabla, type UserActionState } from '../../components/users';
...
export const UsersPage = () => {
  ...
  const handleOpenEditDialog = (use: UserType) => {
    setOpenDialog(true);
    setUser(use);
  };
  ...
  const handleState = async (id: number, status: string) => {
    try {
      console.log(status)
      ...
      showAlert('El usuario fue modificado', 'success');
      listUserApi();
    } catch (error) {
      showAlert(errorHelper(error), 'error');
    }
